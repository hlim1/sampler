...
\aside{The above is kind of bogus, because a bug that affects every
  run would have been found and fixed by QA long before three thousand
  users had to file crash reports.  I like the fact that truly random
  sampling lets us do this calculation at all, but I'm not sure how to
  present it in a way that leaves the whole approach sounding
  feasible.}

\subsubsection{Single-Function Sampling}
\input{applications/ccured-size}

\aside{This entire section needs to be redone more or less from
  scratch.  Our single-function numbers have changed quite a bit since
  the PLDI draft.}
...
function as counted in Table~\ref{tab:share:static}.  All sites in
...
visualize.  Figure~\ref{fig:share:perimeter} shows absolute running
...
\texttt{perimeter} in Table~\ref{tab:share:density}.
