...
char *xreadline(FILE *fin, char *myname) {
  ...
  while (nl == ((void *)0)) {
    ...
    if (res==((void *)0)) return buf;
    nl = (__extension__ (__builtin_constant_p ('\n') && ('\n') == '\0' ? (char *) __rawmemchr (buf+oldbuflen-1, '\n') : strchr (buf+oldbuflen-1, '\n')));
  }
  *nl = 0;
  return buf;
}
