...
In our setting, the input data are the program invariant counters,
normalized to have unit sample variance.  The $\beta$ values
essentially weigh the relative importance of each feature in the final
outcome.  Since we have many more parameters than data points, we
constrain the possible set of parameter values by penalizing large
$\beta$ values using the L1-norm, $\norm{\vec\beta}_1 = \sum_j
\abs{\beta_j}$.  The model is then trained using stochastic gradient
ascent to reach a local maximum of the penalized log likelihood.  Let
$\mu(x) = P(Y = 1 | x)$.  Then:
%%
\begin{equation*}
  \begin{split}
...
the training set \unknown[I have this for the complete traces, but
what is it for 1/100 sampling?] features never take on a non-zero
value and \unknown[I have this for the complete traces, but what is it
for 1/100 sampling?] of them are always some non-zero constant.  Hence
the effective number of features used in training is 7260.  Sparser
...
Pentium 4 CPU with 1 GB of RAM.  \aside{If we decide to discuss the
  1/1000 data, that took eighteen minutes.}  \aside{1 GB RAM is how
  much the box had, not how much MATLAB used.  Alice, do we know how
  much MATLAB used?}

\aside{Interpretation of logistic regression results in statistical
  terms.  How well does the trained model predict the testing runs?
  Of the mispredicts, how many were runs that got lucky?  And of
  course, any other standard model validation assessments that seem
  appropriate.}

\aside{Interpretation of logistic regression results in software
  engineering terms.  Claim that they direct us to some small region
  of code in a single function, versus 9764 lines of code in all of
  \texttt{bc}.  Description of previously unreported buffer overrun
  bug found on like 176 of \texttt{storage.c}.
