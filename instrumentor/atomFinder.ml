...
class visitor file (tuples : Counters.manager) func =
  object (self)
...
(* 	  in *)
      ...
      let skip_lval lv =
	if ((is_bitfield lv) || (is_register lv)) then
	  true
	else
	  let lh,_ = lv in
	  match lh with
	  | Var(vi) -> not vi.vglob (* Don't skip globals *)
	  | _ -> false
      in
	...
	| [] -> lvals
	| expr::tail ->
	    let lvs = get_lvals expr in
	    list_get_lvals tail lvals@lvs
      in
...
(* 	    | _-> false in *)
	...
	match lvals with
	| [] -> (stmts, cur_selector, stale_selector, isAnyCur, isAnyStale) 
	| lv::tail -> 
	    let cur_tid = get_ctid_lval() in 
...
(* 	      let on_blk = mkBlock[ selector_stmt;  bump_stmt; execute_stmt;  reset_stmt; dummy_sample ] in *)
	...
	match lvals with
	| [] ->
	    mkBlock[execute_stmt2; default_stmt] 
	| _ -> 
