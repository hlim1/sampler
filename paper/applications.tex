...
most cases the penalty is quite modest.  Some benchmarks which perform
...
bug there is a predicate (over program variables) that becomes true
...
We instrument this application is as follows.
...
functions which return status codes from those that do not.  
...
\item[\elim{Elimination by successful counterexample}:] Disregard any
  ...
  must represent predicates which can be true without a subsequent
  program failure.
\end{description}
...
runs so that any predicate which is ever true is likely to
...
non-deterministic with respect to a set of program predicates if none of them
is perfectly correlated with program crashes.  
...
of counter triples.  We abandon \elim{elimination by successful
counterexample}\footnote{Because the bug is non-deterministic, if we
have enough runs no predicates will satisfy \elim{elimination by
successful counterexample}.} in favor of statistical modeling to
identify behavior which is broadly correlated with failure in spite of
...
which behave identically in both successful and failed runs.  The
challenge is to find the few predicates which truly matter.
...
are wild guesses, but that there may be just a few which correctly
...
uninstrumented code.  Again, this is apparently due to effects such as
changes in relative code alignment, cache behavior, measurement noise,
and other unpredictable factors.  Thus, we achieved an important goal:
