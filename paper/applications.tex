...
tests we term \termdef{elimination strategies}:
...
strategies are not independent: \elim{universal falsehood} and
...
with a \elim{successful counterexample}, is in another class altogether.
The \elim{universal falsehood} and \elim{successful counterexample}
sets both only look at successful runs, hence are easily analyzed
together.  But the \elim{lack of failing example} strategy in general
eliminates the most raw number of counts, hence it also makes sense to
combine \elim{successful counterexample} with it.  Doing so in the
...
it might serve to our advantage in other programs.  \elim{Elimination
  by lack of failing coverage}, on the other hand, is an inherently
