...
\aside{The above is kind of bogus, because a bug that affects
  \emph{every} run would have been found and fixed by QA long before
  three thousand users had to file crash reports.  If you're
  interested in bugs that only affect one user in a hundred, or one
  user in a thousand, the number of required runs to reach 95\%
  confidence is going to get pretty big.  I like the fact that truly
  random sampling lets us do this calculation at all, but I'm not sure
  how to present it in a way that leaves the whole approach sounding
  feasible.}
...
In the \texttt{tsp} benchmark, all functions but one perform slightly
...
\aside{That's a pretty depressing note to end the section on, but what
  else can we say?  I've looked at several of the worst performing
  functions in \texttt{go} and they all have large acyclic region
  weights, so amortization should be great.  I really have no
  plausible explanation for the \texttt{go} performance data at this
  time.
  
  Perhaps all of Section~\ref{sec:ccured:single} should be shrunk down
  dramatically.  We might just offer a bullet list of things we know
  we could improve, along with some vague statement that these may not
  be the only sources of slowdown.
  
  Part of the problem, I think, is that we're trying to do two
  different things here.  First, we're trying to mention the idea of
  single-function instrumentation.  Alex feels it is very important
  that we discuss this because if we don't claim to have invented it,
  someone else will come along and think of it themselves.  Second,
  we're trying to say that the performance could be improved in a
  number of straightforward ways.  These two ideas are connected
  because single-function instrumentation makes it easier to find
  certain opportunities for improvement, but that's not a very strong
  tie.
  
  On the other hand, one of the reviewers specifically commented on
  the fact that ``Apparent anomalies in the results are aggressively
  addressed and explained,'' so I'm loathe to remove this.

  Hmm.  I'm just not sure what to do here.}
...
tests we term \termdef{elimination heuristics}:
...
We characterize these as heuristics because they are subject to noise
from random sampling, and also because not all are equally applicable
to all bugs.  For example, \elim{Elimination by successful
counterexample} assumes that the bug is deterministic: whenever the
bad behavior occurs, the program \emph{must} crash.  The first three
heuristics do not have this dependency, but do require a large enough
...
heuristics are not independent: \elim{universal falsehood} and
\elim{lack of failing coverage} each eliminate a subset of the
counters identified by
\elim{lack of failing example}.  Elimination heuristics also vary in
which kinds of runs they exploit: \elim{successful counterexample}
considers only successful runs; \elim{lack of failing example} and
\elim{lack of failing coverage} consider only failures;
\elim{universal falsehood} uses both.
...
\item[\elim{Lack of failing coverage}] discards 554 counter triples
...
glance, \elim{elimination by universal falsehood} is quite effective
while \elim{elimination by successful counterexample} seems rather
poor.  However, these two heuristics test disjoint properties and
...
%Assume that \elim{elimination by universal falsehood} mainly
%identifies foolish guesses that could have been avoided entirely given
%a more refined guessing procedure.  
We have seen that
\elim{elimination by successful counterexample} is quite effective
...
filter the predicate set using \elim{elimination by successful
  counterexample}.  We then add another fifty runs, and another fifty,
and so on in steps up to the full set of 2902 successful runs.  We
repeat this entire process one hundred times to gauge how rapidly one
can expect the predicate set to shrink as more runs arrive
over time.
...
Section~\ref{sec:share:whole}.  Somewhat surprisingly, code with
...
always cause a crash, we abandon strict \elim{elimination by
  successful counterexample} in favor of an approach based on
...
non-deterministic, and \elim{elimination by successful counterexample}
...
(\elim{Elimination by lack of failing example} can eliminate another
...
\aside{Where exactly did we state this as ``our original goal''?}
