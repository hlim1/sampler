...
%% changed x and y_i to a and b_i to avoid confusion with the logistic
%% regression notation later on [axz]
At any direct assignment to a scalar variable $a$, we identify all
other local or global variables $\{ b_1, b_2, \dots, b_n \}$ which are
also in scope and which have the same type.  We then compare $a$ to
each $b_i$, and note whether $a$ was smaller, equal to, or greater
than $b_i$.  Even with very sparse sampling, a direct trace of these
...
equality with null.  One comparison between $a$ and $b_i$, which bumps
...
%% the problem of feature selection in a classification setting.[axz]
...
%% the following paragraphs bear substantial changes from the original
%% version [axz]
...
Feature selection can be achieved by \termdef{regularizing} the
function parameters to ignore most input features.

%% this paragraph should be revisited after we finish the experimental
%% results
...
learn a good classifier, we can maximize the \termdef{log likelihood}
of the training set.
%%
\begin{equation*}
  \begin{split}
...
importance of this \termdef{regularization} term through a
\termdef{regularization parameter} $\lambda$.  The penalized log likelihood
function is:
%%
\begin{equation*}
  \begin{split}
...
Some experimental details are in order.  Our \texttt{bc} dataset
...
%% runs into 2704 to be used for training and 1354 for testing.  Sparser
%% sampling drives counts lower: at 1/1000,
%% the effective number of interesting features drops to 6738.
